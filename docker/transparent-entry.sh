...
# Redirect all HTTP traffic to mitmproxy port 8082
iptables -t nat -A PREROUTING -p tcp --dport 80 -j REDIRECT --to-port 8082
# Redirect all HTTPS traffic to mitmproxy port 8082
...
# Start mitmproxy in transparent mode with our capture script on port 8082
exec mitmdump \
    --mode transparent \
    --listen-port 8082 \
    --showhost \
