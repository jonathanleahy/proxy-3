...
# Redirect all HTTP traffic to mitmproxy port 8081
iptables -t nat -A PREROUTING -p tcp --dport 80 -j REDIRECT --to-port 8081
# Redirect all HTTPS traffic to mitmproxy port 8081
...
# Start mitmproxy in transparent mode with our capture script on port 8081
exec mitmdump \
    --mode transparent \
    --listen-port 8081 \
    --showhost \
